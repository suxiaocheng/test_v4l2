...
static void init_device(void)
{
	...
	case IO_METHOD_READ:
		if (!(cap.capabilities & V4L2_CAP_READWRITE)) {
			fprintf(stderr, "%s does not support read i/o/n",
				dev_name);
			exit(EXIT_FAILURE);
		}
	...
	if (0 == xioctl(fd, VIDIOC_CROPCAP, &cropcap)) {
		...
		if (-1 == xioctl(fd, VIDIOC_S_CROP, &crop)) {
			switch (errno) {
			case EINVAL:
				fprintf(stderr, "%s crop is not support/n",
					dev_name);
	...
	switch (io) {
	case IO_METHOD_READ:
		fprintf(stderr, "use io read\n");
		init_read(fmt.fmt.pix.sizeimage);
		break;

	case IO_METHOD_MMAP:
		fprintf(stderr, "use mmap read\n");
		init_mmap();
		break;

	case IO_METHOD_USERPTR:
		fprintf(stderr, "use user ptr read\n");
...
static const struct option long_options[] = { {"device", required_argument,
					       NULL, 'd'}, {"help", no_argument,
							    NULL, 'h'}, {"mmap",
									 ...
									 NULL,
									 'm'},
    {"read", no_argument, NULL, 'r'}, {"userp", no_argument,
				       NULL, 'u'}, {0, 0, 0, 0}
};
